```{r setup_city_xgboost_m1, include=FALSE}
...
```{r helper-functions_city_xgboost_m1, echo=FALSE}
...
if (nrow(train_complete) < 2 || length(unique(train_complete$is_canceled)) < 2) {
  message("Skipping varImp: insufficient training data or class imbalance.")
  top_features <- setdiff(names(train_complete), "is_canceled")
} else {
  capture.output({
    model_caret <- train(
      is_canceled ~ ., data = train_complete,
      method = "xgbTree",
      trControl = trainControl(method = "cv", number = 3, classProbs = TRUE),
      tuneGrid = expand.grid(
        nrounds = 50, max_depth = 3, eta = 0.1, gamma = 0,
        colsample_bytree = 0.8, min_child_weight = 1, subsample = 0.5
      )
    )
  }, file = nullfile())
  importance <- varImp(model_caret)$importance
  top_features <- rownames(importance)[order(importance$Overall, decreasing = TRUE)][1:min(10, nrow(importance))]
}
...
# 假設想要達到 1 : 0.8 的 SMOTE 平衡比例
# 意即：minority_class_count * 0.8 ≈ majority_class_count
target_ratio <- 0.8
...
# downsample_data <- train_selected
# downsample_data$is_canceled <- as.numeric(downsample_data$is_canceled == "canceled")  # 1: canceled, 0: not_canceled
# downsample_train <- apply_downsample_with_ratio(downsample_data[complete.cases(downsample_data), ], target_ratio = 1.5)
...
write.csv(city_results, "result_city_xgb_m1.csv", row.names = FALSE)
...
```{r fig.cap="Prediction Probability Distribution for Canceled vs Not Canceled", warning=FALSE, message=FALSE}
library(ggplot2)
